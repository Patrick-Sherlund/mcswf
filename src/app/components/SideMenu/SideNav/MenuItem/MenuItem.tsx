...
const MenuItem: FC<MenuItemProps> = (menuItemProps: MenuItemProps) => {
    ...
    return (<Link tab-index={tabIndex}
                  ...
                  duration={300}>
        {isSelected
            ? <SelectedListItem variants={framerMotionVariants} whileHover={{scale: 1.1}} whileTap={{scale: 0.95}}>
                <MiniDivider/>
                <SelectedItemText> {pageName} </SelectedItemText>
            </SelectedListItem>
            : <MenuListItem variants={framerMotionVariants} whileHover={{scale: 1.1}} whileTap={{scale: 0.95}}>
